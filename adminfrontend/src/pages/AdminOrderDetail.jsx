...
import {
  FiArrowLeft, FiPrinter, FiDownload,
  FiPackage, FiUser, FiMapPin, FiCreditCard, FiShoppingBag,
  ...
  const handleDownloadInvoice = async () => {
    try {
      ...
      const a = Object.assign(document.createElement("a"), {
        href: url, download: `invoice_${id.slice(-8)}.pdf`,
      });
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-4 sm:p-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div className="flex gap-2">
              ...
              >
                <FiDownload size={14} />
                {downloading ? "Downloading..." : <span className="hidden sm:inline">Download Invoice</span>}
                {downloading ? "..." : <span className="sm:hidden">Invoice</span>}
              </button>
            </div>
          </div>
        </div>
              ...
              <div className="p-4 space-y-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-r from-indigo-500 to-purple-500 flex items-center justify-center text-white text-sm font-bold flex-shrink-0">
                    {order.userId?.name?.slice(0,2).toUpperCase() || "G"}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-semibold text-gray-800">{order.userId?.name || "Guest User"}</p>
                    <p className="text-xs text-gray-500 break-all">{order.userId?.email || "No email provided"}</p>
                  </div>
                </div>
                {order.address?.phone && (
