...
// Add this to the beginning of your AdminDashboard component for better loading skeleton
const DashboardSkeleton = () => (
  <div className="space-y-8 animate-pulse">
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
      {[...Array(4)].map((_, i) => (
        <div key={i} className="bg-white rounded-2xl border border-dark-200 shadow-card p-6">
          <div className="h-3 w-24 bg-dark-200 rounded mb-3"></div>
          <div className="h-8 w-32 bg-dark-200 rounded mb-2"></div>
          <div className="h-3 w-20 bg-dark-200 rounded"></div>
        </div>
      ))}
    </div>
    <div className="bg-white rounded-2xl border border-dark-200 shadow-card p-6">
      <div className="h-4 w-40 bg-dark-200 rounded mb-5"></div>
      <div className="grid grid-cols-2 sm:grid-cols-5 gap-4">
        {[...Array(5)].map((_, i) => (
          <div key={i} className="h-20 bg-dark-200 rounded-xl"></div>
        ))}
      </div>
    </div>
  </div>
          ...
          <div className="p-6 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
            {stats.orderStats &&
              Object.entries(stats.orderStats).map(([status, count]) => (
                ...
                >
                  <span
                    className={`w-2.5 h-2.5 rounded-full mb-3 ${STATUS_DOT[status] || "bg-gray-400"}`}
                  />
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2 mb-0.5">
                        <p className="text-[10px] font-mono font-semibold text-gray-400 tracking-wider">
