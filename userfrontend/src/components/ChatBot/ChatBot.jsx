...
const ChatBot = () => {
  ...
  const handleSend = async (text) => {
    ...
    setMsg(prev => [...prev, { role: "user", text, time }])
    setLoading(true)

    try {
      const res = await sendMessage(text, aiMode)
      ...
      if (error.response?.status === 401) errorMsg = "🔒 Please login to use this feature."
      else if (error.response?.status === 500) errorMsg = "⚠️ Server error. Please try again later."
      else if (error.code === "ECONNABORTED") errorMsg = "⏰ Request timeout. Please check your connection."
      setMsg(prev => [...prev, { role: "bot", text: errorMsg, time }])
    } finally {
      setLoading(false)
    }
  }

  const handleQuickReply = (message) => handleSend(message)
                    ...
                    {!showOrderIdInput ? (
                      ...
                      <div className="flex gap-1 items-center">
                        <input
                          ...
                          autoFocus
                        />
                        <button onClick={handleCancelOrder} className="text-xs px-2 py-1 bg-red-500 text-white rounded-full">✓</button>
                        <button onClick={() => setShowOrderIdInput(false)} className="text-xs px-2 py-1 bg-gray-300 rounded-full">✗</button>
                      </div>
                    )}
                  </div>
                )
                    ...
                    {!showSearchInput ? (
                      ...
                      <div className="flex gap-1 items-center">
                        <input
                          ...
                          autoFocus
                        />
                        <button onClick={handleSearchProduct} className="text-xs px-2 py-1 bg-green-500 text-white rounded-full">🔍</button>
                        <button onClick={() => setShowSearchInput(false)} className="text-xs px-2 py-1 bg-gray-300 rounded-full">✗</button>
                      </div>
                    )}
                  </div>
                )
