...
const OrdersPage = () => {
  ...
  // ✅ moved here — hooks must always be at the top level of the component
  // NOTE: useSearchParams here reads the current page's URL, not an order detail page.
  // The ?action=return param is meant for OrderDetailPage, not OrdersPage.
  // Remove these two lines from here entirely — they don't belong in OrdersPage.
  ...
  const handleBuyAgain = async (item) => {
    try {
      ...
      alert(err?.response?.data?.message || "Could not add to cart. Please try again.");
    } finally {
      setAddingItem(null);
    }
  };
              ...
              <div key={order._id} className="bg-white border border-gray-200 rounded overflow-hidden">
                ...
                <div className="bg-gray-50 border-b border-gray-200 px-4 py-2.5
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <Link
                      to={`/orders/${order._id}?tab=invoice`}
                        ...
                        <tr key={idx} className="border-t border-gray-100 hover:bg-gray-50 transition">
                            ...
                            <div className="flex items-center gap-3">
                              ...
                              <div className="min-w-0">
                                <Link
                                  ...
                                  className="text-sm font-medium text-gray-900 hover:text-[#2874f0] transition line-clamp-2"
                                >
                                  {item.name}
                                </Link>
                                <p className="text-xs text-gray-400 mt-0.5">
                                  Seller: {item.seller || "ShopMart"}
                                </p>
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="px-4 py-3 text-center">
                            ...
                            <p className="text-[10px] text-gray-400 mt-0.5">
                              {order.status === "Delivered" && "Package received"}
                              {order.status === "Shipped"   && "Out for delivery"}
                              {order.status === "Pending"   && "Order confirmed"}
                              {order.status === "Confirmed" && "Being packed"}
                              {isCancelled && `Cancelled on ${new Date(order.updatedAt).toLocaleDateString("en-IN", { day: "numeric", month: "short" })}`}
                            </p>
                          </td>
                            ...
                            {!isCancelled ? (
                              ...
                              isCancelled && order.paymentMethod !== "COD" && (
                                <span className="text-red-600 font-medium">
                                  Refund ₹{order.total} → wallet
                                </span>
                              )
                            )}
                          </td>

                          {/* Actions — ✅ all closing tags fixed */}
                          <td className="px-4 py-3 text-center">
                            <div className="flex flex-col items-center gap-1.5">
                              ...
                              {order.status === "Delivered" && (
                                <Link
                                  to={`/product/${item.productId}/review?orderId=${order._id}`}
                                  className="text-xs border border-[#2874f0] text-[#2874f0] px-2.5 py-1 rounded hover:bg-blue-50 transition whitespace-nowrap"
                                >
                                  Rate &amp; Review
                                </Link>
                              )}

                              {/* ✅ Return — only for Delivered and not already requested */}
                              {order.status === "Delivered" && !order.returnRequested && (
                                <Link
                                  to={`/orders/${order._id}?action=return`}
                                  className="text-xs border border-green-600 text-green-700 px-2.5 py-1 rounded hover:bg-green-50 transition whitespace-nowrap"
                                >
                                  Return
                                </Link>
                              )}

                              {/* ✅ Buy Again — always visible */}
                              <button
                                onClick={() => handleBuyAgain(item)}
                                disabled={addingItem === item.productId}
                                className="text-xs border border-orange-500 text-orange-600 px-2.5 py-1 rounded hover:bg-orange-50 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1"
                              >
